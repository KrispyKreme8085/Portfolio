...
export default function Home({onClose}: MinesweeperProps) {
  ...
  // Generate grid
  const initGrid = () => {
    let cells: Cell[] = Array.from({ length: totalBoxes }, () => ({
      isBomb: false,
      isRevealed: false,
      isFlagged: false,
      adjacent: 0,
    ...
    while (bombsPlaced < bombCount) {
      let i = Math.floor(Math.random() * totalBoxes);
      if (!cells[i].isBomb) {
        cells[i].isBomb = true;
        bombsPlaced++;
      }
    ...
    // Calculate adjacency
    const dirs = [-1, 1, -boxesW, boxesW, -boxesW - 1, -boxesW + 1, boxesW - 1, boxesW + 1];
  ...
  const revealCell = (i: number) => {
    ...
    // Auto reveal neighbors if empty
    if (cell.adjacent === 0) {
      const dirs = [-1, 1, -boxesW, boxesW, -boxesW - 1, -boxesW + 1, boxesW - 1, boxesW + 1];
      dirs.forEach((d) => {
        const ni = i + d;
        ...
        const ny = Math.floor(ni / boxesW);
        if (nx >= 0 && nx < boxesW && ny >= 0 && ny < boxesH) {
          if (!newGrid[ni].isRevealed) revealCell(ni);
        }
      });
    }
    ...
    <div className={styles.popUp}>
        ...
        <div className={styles.background}>
            ...
            >
                ...
                >
                    {cell.isRevealed && !cell.isBomb && cell.adjacent > 0 ? cell.adjacent : ""}
                    {!cell.isRevealed && cell.isFlagged ? <Image src="/images/sweeperFlag.png" alt="Flag" width={15} height={17}/> : ""}
                </div>
                ))}
            </div>
        </div>
